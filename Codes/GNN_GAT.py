...
def visualize_attention(data, model, nodes, id_to_idx, sample_size=500):

    model.eval()
    with torch.no_grad():
        _ = model(data.x, data.edge_index)
        att_weights = model.conv1.attention_weights 
        att_weights_avg = att_weights.mean(dim=1).cpu().numpy() 
    ...
    node_labels = {idx: nodes[list(nodes.keys())[list(id_to_idx.values()).index(idx)]]['labels'][0] for idx in subG.nodes()}  
    node_colors = []
    ...
    nx.draw_networkx_nodes(subG, pos, node_size=500, ax=ax, node_color=node_colors) 
    nx.draw_networkx_labels(subG, pos, labels=node_labels, font_size=10, ax=ax)  
    ...
    ax.set_title("Attention Weights Visualization on Sampled Subgraph", fontsize=16)  
    plt.axis('off')
