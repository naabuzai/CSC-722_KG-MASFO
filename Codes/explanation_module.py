...
class EnhancedExplainability: 
    def __init__(self, model_name="llama3", k_features=3): 
        self.k_features = k_features
        try:
            self.llm = OllamaLLM(model=model_name) 
            logging.info(f"Successfully initialized EnhancedExplainability with {model_name}")
        except Exception as e:
            logging.error(f"Failed to initialize EnhancedExplainability: {e}")
            self.llm = None
            
        self.threshold_functions = {
            "temp": lambda x: "cold" if x < 18 else "optimal" if x < 28 else "hot",
            "tempmin": lambda x: "freezing" if x < 5 else "cold" if x < 15 else "normal",
            "tempmax": lambda x: "mild" if x < 25 else "warm" if x < 35 else "extreme",
            "Humidity": lambda x: "dry" if x < 30 else "optimal" if x < 70 else "humid",
            
            "Water Retention (%)": lambda x: "low" if x < 40 else "good" if x < 70 else "high",
            "Organic Matter (%)": lambda x: "deficient" if x < 2 else "adequate" if x < 4 else "rich",
            "Cation Exchange Capacity (cmol/kg)": lambda x: "low" if x < 10 else "moderate" if x < 25 else "high",
            "Soil Moisture (%)": lambda x: "dry" if x < 20 else "adequate" if x < 60 else "saturated",
            
            "Expected Yield (tons/ha)": lambda x: "low" if x < 3 else "average" if x < 6 else "high",
            "Growth Percentage (%)": lambda x: "early" if x < 30 else "mid" if x < 70 else "mature"
        }
        
        self.agent_features = {
            0: ["Humidity", "temp", "tempmin", "tempmax"],  
            1: ["Expected Yield (tons/ha)", "Growth Percentage (%)"],  
            2: ["Water Retention (%)", "Organic Matter (%)", 
                "Cation Exchange Capacity (cmol/kg)", "Soil Moisture (%)"]  
        }
     
    def compute_feature_importance(self, agents, inputs, prediction, edge_index):
        importance_scores = {}
        top_features = {}
        
        for agent_idx, agent in enumerate(agents):
            agent.eval()
            input_tensor = inputs[agent_idx].requires_grad_(True)
            
            embedding = agent(input_tensor, edge_index)
            
            pseudo_output = embedding.mean(dim=1).sum()  
            
            gradients = torch.autograd.grad(pseudo_output, input_tensor, 
                                          retain_graph=True, create_graph=False)[0]
            
            importance = torch.abs(gradients).mean(dim=0)  
            
            top_k_indices = torch.topk(importance, min(self.k_features, len(importance))).indices
            
            importance_scores[agent_idx] = importance.detach().cpu().numpy()
            top_features[agent_idx] = top_k_indices.cpu().numpy()
            
        return importance_scores, top_features
    
    def semantic_mapping(self, top_features, sample_features, feature_names):
        semantic_labels = {}
        
        for agent_idx, feature_indices in top_features.items():
            agent_labels = []
            agent_feature_names = self.agent_features[agent_idx]
            
            for feat_idx in feature_indices:
                if feat_idx < len(agent_feature_names):
                    feature_name = agent_feature_names[feat_idx]
                    global_idx = feature_names.index(feature_name) if feature_name in feature_names else feat_idx
                    feature_value = sample_features[global_idx] if global_idx < len(sample_features) else 0
                    
                    if feature_name in self.threshold_functions:
                        semantic_label = self.threshold_functions[feature_name](feature_value)
                        agent_labels.append({
            ...
            semantic_labels[agent_idx] = agent_labels
            
        return semantic_labels
    
    def generate_agent_explanations(self, semantic_labels):
        agent_names = ["Weather Agent", "Crop Agent", "Soil Agent"]
        agent_explanations = {}
        
        for agent_idx, labels in semantic_labels.items():
            if agent_idx < len(agent_names):
                agent_name = agent_names[agent_idx]
                conditions = []
                
                for label_info in labels:
                    condition = f"{label_info['label']} {label_info['feature'].lower()} ({label_info['value']:.1f})"
                    conditions.append(condition)
                
                if conditions:
                    explanation = f"{agent_name} detects {', '.join(conditions)}"
                    agent_explanations[agent_idx] = explanation
        
        return agent_explanations
    
    def fuse_explanations(self, agent_explanations, prediction):
        explanation_parts = []
        
        for agent_idx in sorted(agent_explanations.keys()):
            explanation_parts.append(agent_explanations[agent_idx])
        
        pred_str = f"{prediction:.1f} kg/ha" if hasattr(prediction, '__iter__') else f"{prediction:.1f}"
        
        structured_explanation = ". ".join(explanation_parts)
        structured_explanation += f". Through collaborative reasoning, the multi-agent system recommends {pred_str} of fertilizer application."
        
        return structured_explanation
    
    def explain_prediction(self, agents, inputs, prediction, edge_index, sample_features, 
                          feature_names, target_names, true_values=None): 
        if not agents or len(agents) == 0:
            return "No agents available for explanation."
            
        try:
            importance_scores, top_features = self.compute_feature_importance(
                agents, inputs, prediction, edge_index)
            
            semantic_labels = self.semantic_mapping(top_features, sample_features, feature_names)
            agent_explanations = self.generate_agent_explanations(semantic_labels)
            structured_explanation = self.fuse_explanations(agent_explanations, prediction)
      
            if self.llm is not None:
                enhanced_explanation = self.llm_enhancement(
                    structured_explanation, sample_features, prediction, 
                    feature_names, target_names, true_values)
    ...
    def llm_enhancement(self, structured_explanation, features, prediction, 
                       feature_names, target_names, true_values=None):
        formatted_features = {name: value for name, value in zip(feature_names, features)}
        formatted_predictions = {name: value for name, value in zip(target_names, prediction)} if hasattr(prediction, '__iter__') else {"fertilizer": prediction}
        
        prompt = (
            "You are a precision agriculture expert. Based on the systematic analysis below, "
            "provide enhanced insights:\n\n"
            f"Systematic Analysis:\n{structured_explanation}\n\n"
        ...
        prompt += (
            "\nProvide enhanced agricultural guidance including:"
            "\n1. Expected outcomes from this recommendation"
            "\n2. Alternative management strategies"
            "\n3. Timing considerations for application"
            "\n\nKeep response under 250 words and focus on actionable insights."
        )
        
        try:
            explanation = self.llm.invoke(prompt)
            return f"{structured_explanation}\n\nEnhanced Guidance: {explanation}"
        except Exception as e:
            logging.error(f"Failed to generate LLM enhancement: {e}")
            return structured_explanation
...
class LlamaExplainability: 
    ...
    def explain_prediction(self, features, prediction, feature_names, target_names, true_values=None): 
        ...
        prompt += ( 
            ...
            "\n\nKeep response under 200 words and focus on practical insights."
        ) 
        ...
        with torch.no_grad():
            ...
            if test_mask.sum() > 0:
                ...
                if test_indices:
                    ...
                    explainer = EnhancedExplainability(model_name="llama3", k_features=3)
                    sample_idx = test_indices[0]

                    sample_features = X[sample_idx].tolist()
                    sample_prediction = test_pred[0].mean() if hasattr(test_pred[0], '__len__') else test_pred[0]
                    sample_truth = test_true[0].tolist()

                    agent_inputs = [
                        X_weather[sample_idx:sample_idx+1], 
                        X_crop[sample_idx:sample_idx+1],       
                        X_soil[sample_idx:sample_idx+1]      
                    ]

                    explanation = explainer.explain_prediction(
                        agents=[agent_weather, agent_crop, agent_soil],
                        inputs=agent_inputs,
                        ...
                        true_values=sample_truth
                    )

                    logging.info("Enhanced Systematic Explanation:")
